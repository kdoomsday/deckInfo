...
object SingleGroupCardTypeDeckGrouping extends DeckGrouping {
  ...
  override def group(deck: Deck): Map[String, Seq[DeckEntry]] =
    ...
    deck
      .cards
      .foldLeft(initial) {
        case (acc, de) if de.card.is(Creature)     =>
          acc + (s(Creature) -> acc(s(Creature)).appended(de))
        case (acc, de) if de.card.is(Planeswalker) =>
          acc + (s(Planeswalker) -> acc(s(Planeswalker)).appended(de))
        case (acc, de) if de.card.is(Artifact)     =>
          acc + (s(Artifact) -> acc(s(Artifact)).appended(de))
        case (acc, de) if de.card.is(Enchantment)  =>
          acc + (s(Enchantment) -> acc(s(Enchantment)).appended(de))
