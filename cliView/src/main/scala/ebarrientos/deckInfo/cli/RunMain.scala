...
object RunMain extends App {
  ...
  final def run(args: List[String]) =
    logic(args).fold(_ => ExitCode(1), _ => ExitCode(0))
  ...
  def printCurve(curve: Seq[(Int, Int)]) =
    ...
    ZIO.foreach(curve) { case (cost, amnt) =>
      putStrLn(s"$cost: ${"#" * amnt} ($amnt)")
    }

  def printSymbols(symbols: Map[String, Double]) =
    ZIO.foreach_(symbols) { case (symbol, amnt) => putStrLn(s"$symbol: $amnt") }
}
