...
* Deploying to Docker
Deploying is accomplished via ~./mill dockerBuild~. This will generate the image
and tag it with the current release version number.

Doing ~/.mill dockerBuild --release~ will release the current version, and then
advance it by one.

* Running the Docker image
Ensure you map port =8080= so you can access the page. If running in local you can
reach it via http://localhost:8080.

Swagger docs will be in http://localhost:8080/docs.

** Default loaders
The application bases its cards around a ~H2DBQuillLoader~ that runs a
~RacingCardLoader~ as a backup. It will search for cards in the database and, if
not found attempt a base xml file and then loading from the internet via
~MagicIOLoader~. Cards found this way are then stored in the DB for future access.
